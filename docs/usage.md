...
- **`script-update-interval` 不要填 `0`**。填 0 会让 Surge 一直用第一次下载的脚本，之后改多少次远端代码
  都不会生效，删除模块重装也没用（曾经因此排查了一整晚：设备上跑的还是好几个版本之前的代码，表现出来
  却像是各种别的 bug）。用一个正数，本仓库统一填 `3600`。
- **脚本里放一个 `SCRIPT_VERSION` 常量**，让它出现在通知标题或日志里。排查问题时第一件事是确认设备上
  实际跑的是哪一版，不要靠行为反推。
- 万一确认脚本被缓存卡住了：给 `script-path` 加一个查询串（如 `?v=2`）换成新 URL，一定能绕过旧缓存。
